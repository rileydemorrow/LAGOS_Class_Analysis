...
```{r}
#Your code here
ggplot(mean_values_200, aes(x=log10_mean_chl, y=mean_secchi))+
  geom_point()+
  theme_minimal()
...
## Why might this be the case? 
 As chlorophyll increases, the secchi depth decreases because the chlorophyll makes the water murkier and harder to see through, thus decreasing the depth that the secchi disc will be able to be seen.
...
spatial_count <- inner_join(spatial_lakes,site_counts %>%
                            distinct(lagoslakeid,.keep_all=T),
                          by='lagoslakeid')
...
counting <- st_join(states, spatial_count)
...
```{r}
## Your code here. 
state_count <- spatial_count %>%
  group_by(state_zoneid) %>%
  summarize(obvs_sum = sum(count))
```
...
mapview(state_count, zcol = 'obvs_sum')
```
